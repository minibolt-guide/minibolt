...
To enable the Lightning Network on your MiniBolt, we install [LND](lightning/lnd.md), the "Lightning Network Daemon". We then set up an automatic [Static Channel Backup](channel-backup.md) to protect ourselves in case of failure of the SSD drives. We'll then add [ThunderHub](lightning/thunderhub.md), a web-based node management tool. Finally, we'll install the [Zeus mobile app](lightning/zeus.md) to make on-chain and LN payments and manage our node while we're on the go. Together, they make operating your node a breeze.
